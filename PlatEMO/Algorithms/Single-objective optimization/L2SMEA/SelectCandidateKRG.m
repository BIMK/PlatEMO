function newOffspring = SelectCandidateKRG(subproblemList,nLinear,D,candidates)
...
%--------------------------------------------------------------------------
    ...
    PopObj = candidates(:,end-nLinear+1:end);
    ...
    for k = 1 : nLinear
        ...
        theta     = 0.05 *(subproblemList{k}.ub-subproblemList{k}.lb)*nLinear;
        ...
        if sum(remain) > 2
            distMatrix  = pdist2(remainDec,remainDec);
            distMatrix(logical(eye(sum(remain)))) = inf;
            distSMatrix = sort(distMatrix,2);
            distFit     = -(distSMatrix(:,1) + 0.1*distSMatrix(:,2));
            [frontNo,~] = NDSort([distFit,remainFit],1);
            select      = 1:1:sum(remain);
            while any(frontNo>1) && length(select) > 2
                delIdx  = find(frontNo>1);
                [~,del] = max(remainFit(delIdx));
                select(delIdx(del)) = [];
                distSMatrix = sort(distMatrix(select,select),2);
                distFit     = -(distSMatrix(:,1) + 0.1*distSMatrix(:,2));
                [frontNo,~] = NDSort([distFit,remainFit(select)],1);
            end
    ...
    if isempty(newOffspring)
        [MR,MI] = min(NDOffspring(:,nLinear+1:end));
        [~,LI]  = min(MR);
