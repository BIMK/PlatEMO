...
%--------------------------------------------------------------------------
    ...
    for k = 0 : nLinear-1
        ...
        srgtOPTKRG          = srgtsKRGSetOptions(newX(select),Archive(select,D+1));
        srgtsKRG            = srgtsKRGFit(srgtOPTKRG);
        subproblem.fobj     = @(Dec)srgtsKRGEvaluate(Dec,srgtsKRG);
        subproblem.trainDec = newX(select);
        subproblem.maxTheta = mean(Archive(select,D+1));
