function GLMO_SMPSO(Global, numberOfGroups, typeOfGroups)
% ----------------------------------------------------------------------- 
...
% -----------------------------------------------------------------------
    ...
    Pbest            = Population;
    [Gbest,CrowdDis] = GLMO_UpdateGbest(Population,Global.N);

    %% Optimization
    while Global.NotTermination(Gbest)
        Population       = GLMO_SMPSOOperator(Population,Pbest,Gbest(TournamentSelection(2,Global.N,-CrowdDis)), numberOfGroups, typeOfGroups);
        [Gbest,CrowdDis] = GLMO_UpdateGbest([Gbest,Population],Global.N);
