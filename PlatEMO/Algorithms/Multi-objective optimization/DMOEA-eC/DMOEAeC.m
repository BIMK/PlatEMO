...
%--------------------------------------------------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            %% Parameter setting
            INm = Algorithm.ParameterSet(0.2);

            %% Generate the weight vectors
            [W,N] = UniformPoint(Problem.N,Problem.M-1,'grid');
            ...
            [Archive,znad] = UpdateArchive(Population,Problem.N);
            ...
            while Algorithm.NotTerminated(Archive)
                if ~mod(gen,ceil(INm*Problem.maxFE/N))
                ...
                for subgeneration = 1 : 5
                    ...
                    % Update the archive
                    [Archive,znad] = UpdateArchive([Archive,Offspring],Problem.N);
                end
                gen = gen + 1;
            end
        end
