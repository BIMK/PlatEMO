        ...
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)
                ...
                for i = 1 : Problem.N
                    ...
                    if any(~INF)
                        g_old(~INF) = sum(((Population(P(~INF)).objs-repmat(z,sum(~INF),1))./repmat(znad-z,sum(~INF),1)./W(P(~INF),:)).^repmat(p(P(~INF)),1,Problem.M),2).^(1./p(P(~INF)));
                        g_new(~INF) = sum((repmat((Offspring.obj-z)./(znad-z),sum(~INF),1)./W(P(~INF),:)).^repmat(p(P(~INF)),1,Problem.M),2).^(1./p(P(~INF)));
                    end
                ...
                nObj = (Population.objs-repmat(z,Problem.N,1))./repmat(znad-z,Problem.N,1);
                for i = find(rand(1,Problem.N)>=Problem.FE/Problem.maxFE)
                    g = zeros(Problem.N,length(P));
                    for j = 1 : length(P)
                        if P(j) ~= inf
                            g(:,j) = sum((nObj./repmat(W(i,:),Problem.N,1)).^P(j),2).^(1./P(j));
                        else
                            g(:,j) = max((nObj./repmat(W(i,:),Problem.N,1)),[],2);
                        end
                    end
                    [~,ZK] = min(g,[],1);
