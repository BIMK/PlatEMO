 function newPOP = EnvironmentalSelectionIII(Population,N,Z,Zmin)
...
%--------------------------------------------------------------------------
    ...
    Choose = LastSelection(OBJS(Next,:),OBJS(Last,:),N-sum(Next),Z,Zmin);
    Next(Last(Choose)) = true;
    % Population for next generation
    newPOP = struct();
    newPOP.decs = DECS(Next,:);
    newPOP.objs = OBJS(Next,:);
end
...
% Select part of the solutions in the last front
    ...
    % Normalization
    PopObj = PopObj./repmat(max(PopObj(Extreme,:),[],1),N,1);
