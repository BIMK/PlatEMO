function Arc= WeightOptimization(Global,G2,Population,wD,N)
...
% This function is written by Cheng He
    ...
    [fitness,PopNew] = fitfunc(w0,Direct,Global,Reference);	% Calculate the fitness and store the solutions
    ...
    for it = 1 : G2
        for i = 1 : N
            ...
            NewSol.Position = z;
            [fit,PopNew] = fitfunc(z,Direct,Global,Reference);
            temp = [temp,PopNew];
    ...
    Arc = [Arc,temp];
    if length(Arc) > Global.N
        [frontNo,~] = NDSort(Arc.objs,1);
        Arc = Arc(frontNo==1);
    end
end

function [Obj,OffSpring] = fitfunc(w0,direct,Global,Reference)
    [SubN,WD] = size(w0); 
    WD      = WD/2;
    Obj   	= zeros(SubN,1);
    OffSpring  = [];
