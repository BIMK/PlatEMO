function [P_temp, P_temp_rank] = OSP(ger_end, Pt, Pt_apt, Pt_Rank, Global, ger_init, p)
...
% This function is written by Elaine Guerrero-Pena
    ...
    X_f = fun_Otimiz(indiv_S,phi_S,phi_f,Global);
    ...
    if ksol~=0
        X_LS = fun_GMMOV(X_f.decs, Global, ksol);
        P_temp = [X_f X_LS];
    else
        P_temp = X_f;
    end

    P_temp_rank = NDSort(P_temp.objs,P_temp.cons,Global.N);
...
function XF = fun_Otimiz(indiv_Max,apt_Max,yF,Global)
    ...
    for i=1:size(yF,1)
        ...
        fun = @(x) pdist2(INDIVIDUAL(x).obj,yF(i,:));

        XF_temp = INDIVIDUAL(x0);
        ...
        [x, dist_aptx] = fmincon(fun,x0,[],[],[],[],...
            Global.lower,Global.upper,[],options);

        if dist_aptx < dist_apt
            XF_temp = INDIVIDUAL(x);
        end
...
function Y_LS = fun_GMMOV(indiv, Global, ksol)
    ...
    Y = fun_GMMRand(GMModel,Global.D,ksol);
