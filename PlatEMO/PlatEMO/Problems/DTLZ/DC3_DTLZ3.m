...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopCon = CalCon(obj,PopDec)
            M = obj.Global.M;
            PopCon(:,1:M-1) = 0.5 - cos(3*pi*PopDec(:,1:M-1));
            g = 10*(obj.Global.D-M+1+sum((PopDec(:,M:end)-0.5).^2-cos(20.*pi.*(PopDec(:,M:end)-0.5)),2));
            PopCon(:,M) = 0.5 - cos(3*pi*g);
        end
        ...
        function P = PF(obj,N)
            ...
            P(any(cos(3*pi*acos(c(:,1:M-1)))<0.5,2),:) = [];
