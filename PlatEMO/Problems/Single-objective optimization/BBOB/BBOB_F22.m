...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function Setting(obj)
            ...
            alpha = 1000.^(2*(0:19)/19);
            alpha = [1e6,alpha(randperm(RandStream('mlfg6331_64','Seed',2),end))];
            for i = 1 : length(alpha)
                obj.C{i} = diag(alpha(i).^(0.5*(0:obj.D-1)/(obj.D-1)))/alpha(i)^0.25;
            end
            obj.y = [rand(RandStream('mlfg6331_64','Seed',3))*7.84-3.92,rand(RandStream('mlfg6331_64','Seed',4),1,20)*9.8-4.9];
