...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,X)
            Z = Tasy(0.2,Tosz((X-repmat(obj.xopt,size(X,1),1))*obj.R))*(obj.R*diag(10.^(0.5*(0:obj.D-1)/(obj.D-1)))*obj.Q);
            PopObj = 10*(obj.D-sum(cos(2*pi*Z),2)) + sum(Z.^2,2);
        end
    end
end
