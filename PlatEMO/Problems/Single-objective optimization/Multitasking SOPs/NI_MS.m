...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function Setting(obj)
            ...
            obj.L2       = zeros(1,obj.SubD(2)) - 0.5;
            obj.U2       = zeros(1,obj.SubD(2)) + 0.5;
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 1 : size(PopDec,1)
                ...
                elseif PopDec(i,end) == 2   % Task 2
                    ...
                    for k1 = 1 : length(x2)
                        for k2 = 0 : kmax
                            PopObj(i) = PopObj(i) + a^k2*cos(2*pi*b^k2*(x2(k1)+0.5));
                        end
                    end
                    for k2 = 0 : kmax
                        PopObj(i) = PopObj(i) - length(x2)*a^k2*cos(2*pi*b^k2*0.5);
                    end
                end
            end
        end
