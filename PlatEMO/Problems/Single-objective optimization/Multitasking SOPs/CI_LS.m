classdef CI_LS < PROBLEM
% <single> <real> <large/none> <multitask>
% Multitasking problem (Ackley function + Schwefel function)
...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function Setting(obj)
            ...
            obj.L1       = zeros(1,obj.SubD(1)) - 32;
            obj.U1       = zeros(1,obj.SubD(1)) + 32;
            obj.L2       = zeros(1,obj.SubD(2)) - 500;
            obj.U2       = zeros(1,obj.SubD(2)) + 500;
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 1 : size(PopDec,1)
                if PopDec(i,end) == 1       % Task 1
                    ...
                    PopObj(i) = -20*exp(-0.2*sqrt(mean(x1.^2,2))) - exp(mean(cos(2*pi*x1),2)) + 20 + exp(1);
                elseif PopDec(i,end) == 2   % Task 2
                    x2 = obj.L2 + PopDec(i,1:obj.SubD(2)).*(obj.U2-obj.L2);
                    PopObj(i) = -sum(x2.*sin(sqrt(abs(x2))),2);
                end
            end
        end        
    end
