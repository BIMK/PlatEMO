    ...
    methods
        ...
        function Setting(obj)
            ...
            if isempty(obj.D) || obj.D < 30
                obj.D   = 10;
                obj.Mat = Data{11}.M_10;
            else
                obj.D   = 30;
                obj.Mat = Data{11}.M_30;
            end
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            Y = Z*obj.Mat;
            PopObj = mean((-Y.*cos(2*sqrt(abs(Y)))),2);
        end
        %% Calculate constraint violations
        function PopCon = CalCon(obj,PopDec)
            Z = 1 + PopDec - repmat(obj.O(1:size(PopDec,2)),size(PopDec,1),1);
            PopCon = abs(sum(100*(Z(:,1:end-1).^2-Z(:,2:end)).^2+(Z(:,1:end-1)-1).^2,2)) - 1e-4;
        end
    end
end