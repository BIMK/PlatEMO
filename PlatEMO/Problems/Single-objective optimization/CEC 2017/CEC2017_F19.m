...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj = sum(sqrt(abs(Y))+2*sin(Y.^3),2);
        end
        %% Calculate constraint violations
        function PopCon = CalCon(obj,PopDec)
            Y = PopDec - repmat(obj.O(1:size(PopDec,2)),size(PopDec,1),1);
            PopCon(:,1) = sum(-10*exp(-0.2*sqrt(Y(:,1:end-1).^2+Y(:,2:end).^2)),2) + (size(Y,2)-1)*10./exp(-5);
            PopCon(:,2) = sum(sin(2*Y).^2,2) - 0.5*size(Y,2);
        end
    end
end