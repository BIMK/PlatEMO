...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function Setting(obj)
            ...
            obj.lower    = zeros(1,obj.D) - 20;
            obj.upper    = zeros(1,obj.D) + 20;
        ...
        function PopCon = CalCon(obj,PopDec)
            Z = PopDec - repmat(obj.O(1:size(PopDec,2)),size(PopDec,1),1);
            PopCon(:,1) = abs(sum(Z.*sin(Z),2)) - 1e-4;
            PopCon(:,2) = abs(sum(Z.*sin(pi*Z),2)) - 1e-4;
            PopCon(:,3) = abs(sum(Z.*cos(Z),2)) - 1e-4;
            PopCon(:,4) = abs(sum(Z.*cos(pi*Z),2)) - 1e-4;
            PopCon(:,5) = abs(sum(Z.*sin(2*sqrt(abs(Z))),2)) - 1e-4;
        end
    end
end