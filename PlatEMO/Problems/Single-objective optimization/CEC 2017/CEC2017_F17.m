...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj = 1/4000*sum(Y.^2,2) + 1 - prod(cos(Y./sqrt(repmat(1:size(Y,2),size(Y,1),1))),2);
        end
        %% Calculate constraint violations
        function PopCon = CalCon(obj,PopDec)
            Y = PopDec - repmat(obj.O(1:size(PopDec,2)),size(PopDec,1),1);
            PopCon(:,1) = 1 - sum(sign(abs(Y)-repmat(sum(Y.^2,2),1,size(Y,2))+Y.^2-1),2);
            PopCon(:,2) = abs(sum(Y.^2,2)-4*size(Y,2)) - 1e-4;
        end
    end
end