...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            Z = Z*obj.Mat';
            PopObj = sum(sqrt(abs(Z))+2*sin(Z.^3),2);
        end
        %% Calculate constraint violations
        function PopCon = CalCon(obj,PopDec)
            Z = PopDec - repmat(obj.O(1:size(PopDec,2)),size(PopDec,1),1);
            Z = Z*obj.Mat';
            PopCon(:,1) = sum(-10*exp(-0.2*sqrt(Z(:,1:end-1).^2+Z(:,2:end).^2)),2) + (size(Z,2)-1)*10./exp(-5);
            PopCon(:,2) = sum(sin(2*Z).^2,2) - 0.5*size(Z,2);
        end
    end
end