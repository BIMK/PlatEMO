...
%-------------------------------------------------------------------------- 
    ...
    methods
        %% Initialization
        function obj = Sparse_SR()
            [lenSig,lenObs,sparsity,sigma] = obj.Global.ParameterSet(1024,480,260,0);
            N = lenSig;
            M = lenObs;
            K = sparsity;
            ...
            obj.Global.lower    = floor(repmat(mean(x)-3*std(x),1,obj.Global.D));
            obj.Global.upper    = ceil(ones(1,obj.Global.D)*(mean(x)+3*std(x)));
        ...
        function PopDec = Init(obj, N)
            e = randi(obj.Global.D,1,N);
            PopDec = zeros(N,obj.Global.D);
            for i = 1 : N
                integer       = e(i);	% sparse support
                [~,spar_b]    = sort(rand(1,obj.Global.D));
