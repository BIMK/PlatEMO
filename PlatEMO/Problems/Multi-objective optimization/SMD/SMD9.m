...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function Setting(obj)
            ...
            obj.p  = obj.DU - obj.r;
            obj.q  = obj.DL - obj.r;
            obj.lower    = [-5*ones(1,obj.DU), -5*ones(1,obj.q), -ones(1,obj.r)+1e-6];
            obj.upper    = [10*ones(1,obj.p), ones(1,obj.r), 10*ones(1,obj.q), exp(1)*ones(1,obj.r)-1];
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj(:,1) = sum(xu1.^2,2) - sum(xl1.^2,2) + sum(xu2.^2,2) - sum((xu2-log(1+xl2)).^2,2);
            % Lower level function value
            PopObj(:,2) = sum(xu1.^2,2) + sum(xl1.^2,2) + sum((xu2-log(1+xl2)).^2,2);
        end
