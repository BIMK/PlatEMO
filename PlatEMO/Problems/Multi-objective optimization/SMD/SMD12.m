...
% <2014> <multi> <real> <constrained> <bilevel>
...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function Setting(obj)
            ...
            obj.q  = obj.DL - obj.r;
            obj.C  = obj.p + 2*obj.r;
            obj.lower    = [-5*ones(1,obj.p), -ones(1,obj.r), -5*ones(1,obj.q), -pi/4*ones(1,obj.r)+1e-6];
            obj.upper    = [10*ones(1,obj.p),  ones(1,obj.r), 10*ones(1,obj.q),  pi/4*ones(1,obj.r)-1e-6];
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj(:,1) = sum((xu1-2).^2,2) + sum(xl1.^2,2) + sum((xu2-2).^2,2) + sum(tan(abs(xl2)),2) - sum((xu2-tan(xl2)).^2,2);
        ...
        function PopCon = CalCon(obj,PopDec)
            ...
            PopCon(:,obj.p+obj.r+1:obj.p+2*obj.r) = -xu2 + tan(xl2);
            % Lower level constraint violation
            PopCon(:,obj.p+2*obj.r+1:obj.p+2*obj.r+obj.q) = -xl1 - xl1.^3 + sum(xl1.^3,2);
            PopCon(:,end+1) = -sum((xu2-tan(xl2)).^2,2) + 1;
        end
