...
% This function is written by Kangjia Qiao (email: qiaokangjia@yeah.net)
    ...
    methods
        ...
        function Setting(obj)
            ...
            obj.a = information(15);  % 15 indicates the index of problem
        ...
        function Population = Evaluation(obj,varargin)
            ...
            obj.THETA_=zeros(N,2);
            Sx = cumsum(PopDec(:,1:obj.M).^2,2,'reverse');
            angle = atan(sqrt(Sx(:,2:end))./PopDec(:,1:obj.M-1));
            index = isnan(angle);
            angle(index==1) = 1;
            obj.THETA_ =2./pi.*angle;
            
            % Calculate objectives
            Pop = (obj.q1_upper - obj.q1_lower).* PopDec(:,1:3) + obj.q1_lower;
            T_ = (1-Pop(:,1).^2-Pop(:,2).^2-Pop(:,3).^2).^2 + obj.h + dis_P;
            G_ =  [ones(N,1) cumprod(sin(pi/2*obj.THETA_),2)] .* [cos(pi/2*obj.THETA_) ones(N,1)];
            PopObj =  G_ .* repmat((1+T_),1,obj.M);

            % Calculate constraints
            PopCon(:,1)=  0.5 - (Pop(:,1).^2 + Pop(:,2).^2 + Pop(:,3).^2) ;
            PopCon(:,2)=  -2 - (Pop(:,1).^2 + Pop(:,2).^2 + Pop(:,3).^2 ) ;
        ...
        function R = GetPF(obj)
            ...
            R = {sin(t)*cos(t'),sin(t)*sin(t'),cos(t)*ones(size(t'))};
        end
    end
end
