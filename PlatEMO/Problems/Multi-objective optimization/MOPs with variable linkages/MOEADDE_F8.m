...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,X)
            ...
            PopObj(:,1) = X(:,1)         + 2/length(J1)*(4*sum(Y(:,J1).^2,2)-2*prod(cos(20*Y(:,J1)*pi./repmat(sqrt(J1),size(X,1),1)),2)+2);
            PopObj(:,2) = 1-sqrt(X(:,1)) + 2/length(J2)*(4*sum(Y(:,J2).^2,2)-2*prod(cos(20*Y(:,J2)*pi./repmat(sqrt(J2),size(X,1),1)),2)+2);
        end
