...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            t = ((y(:,1)>=0.1&y(:,1)<=0.4)|(y(:,1)>=0.6&y(:,1)<=0.9))|repmat(obj.M==2,size(y,1),1);
            A = [y(:,1:obj.M-1),sum((0.5-y(:,1:obj.M-1)).^5,2)./2/(obj.M-1)/0.5^5+1/2];
            A(t,:) = [repmat(y(t,1),1,obj.M-1),((0.5-y(t,1)).^5+0.5^5)./2/0.5^5];
        ...
        function R = GetOptimum(obj,N)
            ...
            t = ((x(:,1)>=0.1&x(:,1)<=0.4)|(x(:,1)>=0.6&x(:,1)<=0.9))|repmat(obj.M==2,size(x,1),1);
            R = [x,sum((0.5-x).^5,2)./2/(obj.M-1)/0.5^5+1/2];
            R(t,:) = [repmat(x(t,1),1,obj.M-1),((0.5-x(t,1)).^5+0.5^5)./2/0.5^5];
        ...
        function R = GetPF(obj)
            ...
            elseif obj.M == 3
                ...
                t       = (x>=0.1&x<=0.4) | (x>=0.6&x<=0.9);
                R       = {x,4*y,9*(((0.5-x).^5+(0.5-y).^5)./2/(obj.M-1)/0.5^5+1/2)};
                R{2}(t) = 4*x(t);
                R{3}(t) = 9*((0.5-x(t)).^5+0.5^5)./2/0.5^5;
            else
                R = [];
            end
        end
...
function g = g3(y,theta)
    g = mean(cos(pi*y+theta).^2,2);
end