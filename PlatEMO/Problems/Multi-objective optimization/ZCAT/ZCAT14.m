...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            A = [sin(pi/2*y(:,1)).^2,sin(pi/2*y(:,1)).^(2+(1:obj.M-3)/(obj.M-2))];
            if obj.M > 2
                A = [A,(1+sin(3*pi*y(:,1)-pi/2))/2];
            end
            A = [A,cos(pi/2*y(:,1))];
        ...
        function R = GetOptimum(obj,N)
            x = linspace(0,1,N)';
            R = [sin(pi/2*x).^2,sin(pi/2*x).^(2+(1:obj.M-3)/(obj.M-2))];
            if obj.M > 2
                R = [R,(1+sin(3*pi*x-pi/2))/2];
            end
            R = [R,cos(pi/2*x)];
