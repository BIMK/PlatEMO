...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            t = all(y(:,1:obj.M-1)<=0.5,2);
            A = [y(:,1:obj.M-1),(exp(mean(1-y(:,1:obj.M-1),2)).^8-1)/(exp(1).^8-1)];
            A(t,:) = [repmat(y(t,1),1,obj.M-1),(exp(1-y(t,1)).^8-1)/(exp(1).^8-1)];
        ...
        function R = GetOptimum(obj,N)
            ...
            t = all(x<=0.5,2);
            R = [x,(exp(mean(1-x,2)).^8-1)/(exp(1).^8-1)];
            R(t,:) = [repmat(x(t,1),1,obj.M-1),(exp(1-x(t,1)).^8-1)/(exp(1).^8-1)];
        ...
        function R = GetPF(obj)
            ...
            elseif obj.M == 3
                ...
                t       = x<=0.5 & y<=0.5;
                R       = {x,4*y,9*(exp(1-x/2-y/2).^8-1)/(exp(1).^8-1)};
                R{2}(t) = 4*x(t);
                R{3}(t) = 9*(exp(1-x(t)).^8-1)/(exp(1).^8-1);
            else
                R = [];
            end
        end
