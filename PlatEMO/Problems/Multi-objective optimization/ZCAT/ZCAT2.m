...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            A = repmat((1:obj.M).^2,size(y,1),1).*fliplr(cumprod([ones(size(y,1),1),1-cos(y(:,1:obj.M-1)*pi/2)],2)).*[ones(size(y,1),1),1-sin(y(:,obj.M-1:-1:1)*pi/2)];
        ...
        function R = GetPF(obj)
            ...
            elseif obj.M == 3
                ...
                R = {(1-cos(a))*(1-cos(a')),4*(1-cos(a))*(1-sin(a')),9*(1-sin(a))*ones(size(a'))};
            else
                R = [];
            end
        end
...
function g = g5(y,theta)
    g = mean(sin(2*pi*y-1+theta).^3,2)/2 + 1/2;
end