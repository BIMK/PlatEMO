...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            A = [sin(pi/2*y(:,1)),sin(pi/2*y(:,1)).^(1+(1:obj.M-3)/(obj.M-2))];
            if obj.M > 2
                A = [A,(1+sin(5*pi*y(:,1)-pi/2))/2];
            end
            A = [A,(cos(9*pi*y(:,1))+2*y(:,1)+20*(1-y(:,1))-1)/20];
        ...
        function R = GetOptimum(obj,N)
            x = linspace(0,1,N)';
            R = [sin(pi/2*x),sin(pi/2*x).^(1+(1:obj.M-3)/(obj.M-2))];
            if obj.M > 2
                R = [R,(1+sin(5*pi*x-pi/2))/2];
            end
            R = [R,(cos(9*pi*x)+2*x+20*(1-x)-1)/20];
...
function g = g10(y,theta)
    g = sum(sin((4*y-2)*pi+theta).^3,2)/2/size(y,2)^3 + 1/2;
end