...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            A = fliplr(cumsum(y(:,1:obj.M-1),2)) + [zeros(size(y,1),1),1-y(:,obj.M-1:-1:2)];
            A = [A,(cos(7*pi*y(:,1))+2*y(:,1)+16*(1-y(:,1))-1)/16];
        ...
        function R = GetOptimum(obj,N)
            ...
            R = fliplr(cumsum(x,2)) + [zeros(size(x,1),1),1-x(:,end:-1:2)];
            R = [R,(cos(7*pi*x(:,1))+2*x(:,1)+16*(1-x(:,1))-1)/16];
        ...
        function R = GetPF(obj)
            ...
            elseif obj.M == 3
                ...
                R     = {(x+y)/2,2*(x+1-y),9*(cos(7*pi*x)+2*x+16*(1-x)-1)/16};
