...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            A = fliplr(cumsum([zeros(size(y,1),1),sin(pi/2*y(:,1:obj.M-1))],2)) + [zeros(size(y,1),1),cos(pi/2*y(:,obj.M-1:-1:1))];
            A = repmat((1:obj.M).^2./[obj.M-1,obj.M-1:-1:1],size(y,1),1).*A;
        ...
        function R = GetOptimum(obj,N)
            ...
            R = fliplr(cumsum([zeros(size(x,1),1),sin(pi/2*x)],2)) + [zeros(size(x,1),1),cos(pi/2*x(:,end:-1:1))];
            R = repmat((1:obj.M).^2./[obj.M-1,obj.M-1:-1:1],size(x,1),1).*R;
        end
        ...
        function R = GetPF(obj)
            ...
            elseif obj.M == 3
                ...
                R     = {(sin(pi/2*x)+sin(pi/2*y))/2,2*(sin(pi/2*x)+cos(pi/2*y)),9*cos(pi/2*x)};
            else
                R = [];
            end
        end
...
function g = g9(y,theta)
    g = mean(y,2)/2 - mean(abs(sin(2.5*pi*y-pi/2+theta)))/2 + 1/2;
end