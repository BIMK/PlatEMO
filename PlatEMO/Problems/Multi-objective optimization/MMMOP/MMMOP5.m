...
%--------------------------------------------------------------------------
	...
	methods
        ...
        function PopObj = CalObj(obj,PopDec)
            y    = PopDec(:,1:obj.M-1)*(2^(obj.d+1)-1);
            temp = cumsum(2.^(obj.d:-1:0));
            for i = 1 : size(y,1)
                for j = 1 : size(y,2)
                    index  = find(y(i,j)<=temp,1);
                    y(i,j) = (y(i,j)-temp(index)+2^(obj.d-index+1))/2^(obj.d-index+1);
                end
            end
        ...
        function DrawObj(obj,Population)
            ...
            XA = Grid(0.5/obj.c:1/obj.c:(1-0.5/obj.c),obj.kA);
            a  = cumsum(2.^(obj.d:-1:0))/(2^(obj.d+1)-1);
            X(obj.d+1) = a(obj.d+1);
            for i = obj.d : -1 : 1
                X(i) = 2*a(i) - X(i+1);
            end
