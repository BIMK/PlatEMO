...
%--------------------------------------------------------------------------
	...
	methods
        ...
        function PopObj = CalObj(obj,PopDec)
            y      = PopDec(:,1:obj.M-1).*obj.d-floor(PopDec(:,1:obj.M-1).*obj.d);
            g      = obj.kA+sum(cos(2.*pi.*obj.c.*PopDec(:,obj.M:obj.M-1+obj.kA)),2)+sum((PopDec(:,obj.M+obj.kA:end)-0.5).^2,2);          
        ...
        function R = GetOptimum(obj,N)
            ...
            R  = [repmat(X,size(XA,1),1),XA(repmat(1:end,size(X,1),1),:),zeros(size(X,1)*size(XA,1),obj.D-size(X,2)-size(XA,2))+0.5];
        end
        ...
        function DrawObj(obj,Population)
            ...
            XA = Grid(0.5/obj.c:1/obj.c:(1-0.5/obj.c),obj.kA);
            X  = Grid(0.5/obj.d:1/obj.d:(1-0.5/obj.d),obj.M-1);
