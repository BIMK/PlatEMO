...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopCon = CalCon(obj,PopDec)
            PopCon(:,1:obj.M-1) = 0.5 - cos(3*pi*PopDec(:,1:obj.M-1));
            g = 100*(obj.D-obj.M+1+sum((PopDec(:,obj.M:end)-0.5).^2-cos(20.*pi.*(PopDec(:,obj.M:end)-0.5)),2));
            PopCon(:,obj.M) = 0.5 - cos(3*pi*g);
        end
        ...
        function R = GetOptimum(obj,N)
            ...
            R(any(cos(3*pi*x(:,1:obj.M-1))<0.5,2),:) = [];
            R = R/2;
        end
        ...
        function R = GetPF(obj)
            ...
            elseif obj.M == 3
                ...
                y = a*(1-a');
                z = (1-a)*ones(size(a'));
                z(cos(3*pi*a*ones(size(a')))<0.5 | cos(3*pi*ones(size(a))*a')<0.5) = nan;
                R = {x/2,y/2,z/2};
            else
                R = [];
            end
