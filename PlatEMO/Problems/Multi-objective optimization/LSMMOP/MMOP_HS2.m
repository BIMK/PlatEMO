...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            sk5 = find(PopDec(:,end)==1);
            X5  = PopDec(sk5,1:obj.SubD(1));
            X5  = (obj.L{1} + (obj.U{1} - obj.L{1}) .* X5) .* (X5 ~= 0);
            K5  = ceil(obj.SubTheta(1)*(obj.SubD(1)-obj.M+1));
            g55 = sum(g1(X5(:,obj.M:end),pi/3).*g2(X5(:,obj.M:end),0),2) + abs(K5-sum(X5(:,obj.M:end)~=0,2));
            PopObj(sk5,:) = repmat(1+g55/(obj.SubD(1)-obj.M+1),1,obj.M).*fliplr(cumprod([ones(size(X5,1),1),1-cos(X5(:,1:obj.M-1)*pi/2)],2)).*[ones(size(X5,1),1),1-sin(X5(:,obj.M-1:-1:1)*pi/2)];

            sk6 = find(PopDec(:,end)==2);
            X6  = PopDec(sk6,1:obj.SubD(2));
            X6  = (obj.L{2} + (obj.U{2} - obj.L{2}) .* X6) .* (X6 ~= 0);
            K6  = ceil(obj.SubTheta(2)*(obj.SubD(2)-obj.M+1));
            g66 = g4(X6(:,obj.M:end),repmat(linspace(0,1,obj.SubD(2)-obj.M+1),size(X6,1),1));
            [g66,rank] = sort(g66,2);
            ...
            g66(temp)    = 0;
            g66 = sum(g66,2);
            PopObj(sk6,:) = repmat(1+g66/(obj.SubD(2)-obj.M+1),1,obj.M).*fliplr(cumprod([ones(size(X6,1),1),1-cos(X6(:,1:obj.M-1)*pi/2)],2)).*[ones(size(X6,1),1),1-sin(X6(:,obj.M-1:-1:1)*pi/2)];
        end
