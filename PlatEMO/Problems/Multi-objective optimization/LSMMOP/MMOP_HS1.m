...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            sk2 = find(PopDec(:,end)==1);
            X2  = PopDec(sk2,1:obj.SubD(1));
            X2  = (obj.L{1} + (obj.U{1} - obj.L{1}) .* X2) .* (X2 ~= 0);
            K2  = ceil(obj.SubTheta(1)*(obj.SubD(1)-obj.M+1));
            g22 = sum(g2(X2(:,obj.M:obj.M+K2-1),pi/3),2) + sum(g3(X2(:,obj.M+K2:end),0),2);
            PopObj(sk2,:) = repmat(1+g22/(obj.SubD(1)-obj.M+1),1,obj.M).*fliplr(cumprod([ones(size(X2,1),1),X2(:,1:obj.M-1)],2)).*[ones(size(X2,1),1),1-X2(:,obj.M-1:-1:1)];

            sk4 = find(PopDec(:,end)==2);
            X4  = PopDec(sk4,1:obj.SubD(2));
            X4  = (obj.L{2} + (obj.U{2} - obj.L{2}) .* X4) .* (X4 ~= 0);
            [N4,D4] = size(X4);
            M   = obj.M;
            K4  = ceil(obj.SubTheta(2)*(D4-M+1));
            g44 = sort(g3(X4(:,M:end),0),2);
            g44 = sum(g44(:,1:D4-M-K4+1),2);
            PopObj(sk4,:) = repmat(1+g44/(D4-M+1),1,M).*fliplr(cumprod([ones(N4,1),1-cos(X4(:,1:M-1)*pi/2)],2)).*[ones(N4,1),1-sin(X4(:,M-1:-1:1)*pi/2)];                                                           
        end
