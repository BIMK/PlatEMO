...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            sk3 = find(PopDec(:,end)==1);
            X3  = PopDec(sk3,1:obj.SubD(1));
            X3  = (obj.L{1} + (obj.U{1} - obj.L{1}) .* X3) .* (X3 ~= 0);
            K3  = ceil(obj.SubTheta(1)*(obj.SubD(1)-obj.M+1));
            g33 = sum(g1(X3(:,obj.M:obj.M+K3-1),pi/3),2);
            for i = 1 : ceil((obj.SubD(1)-obj.M-K3+1)/10)
                temp = 50 - sum(g1(X3(:,obj.M+K3+(i-1)*10:min(obj.M+K3+i*10-1,end)),0),2);
                g33(temp<50) = g33(temp<50) + temp(temp<50);
            end
            PopObj(sk3,:) = repmat(1+g33/(obj.SubD(1)-obj.M+1),1,obj.M).*fliplr(cumprod([ones(size(X3,1),1),X3(:,1:obj.M-1)],2)).*[ones(size(X3,1),1),1-X3(:,obj.M-1:-1:1)];

            sk7 = find(PopDec(:,end)==2);
            X7  = PopDec(sk7,1:obj.SubD(2));
            X7  = (obj.L{2} + (obj.U{2} - obj.L{2}) .* X7) .* (X7 ~= 0);
            K7  = ceil(obj.SubTheta(2)*(obj.SubD(2)-obj.M+1));
            g77 = sum(g2(X7(:,obj.M:obj.M+K7-1),pi/3),2) + sum(g2(X7(:,obj.M+K7:end),X7(:,[obj.M+K7+1:end,obj.M+K7])*0.9),2);
            PopObj(sk7,:) = repmat(1+g77/(obj.SubD(2)-obj.M+1),1,obj.M).*fliplr(cumprod([ones(size(X7,1),1),cos(X7(:,1:obj.M-1)*pi/2)],2)).*[ones(size(X7,1),1),sin(X7(:,obj.M-1:-1:1)*pi/2)];
        end
