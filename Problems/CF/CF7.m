function varargout = CF7(Operation,Global,input)
% <problem> <CF>
...
%--------------------------------------------------------------------------
    ...
    switch Operation
        ...
        case 'value'
            ...
            PopCon(:,1) = -X(:,2)+sin(6*pi*X(:,1)+2*pi/D)+sign(0.5*(1-X(:,1))-(1-X(:,1)).^2).*sqrt(abs(0.5*(1-X(:,1))-(1-X(:,1)).^2));
            PopCon(:,2) = -X(:,4)+sin(6*pi*X(:,1)+4*pi/D)+sign(0.25*sqrt(1-X(:,1))-0.5*(1-X(:,1))).*sqrt(abs(0.25*sqrt(1-X(:,1))-0.5*(1-X(:,1))));
