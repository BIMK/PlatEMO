function varargout = MOEADDE_F4(Operation,Global,input)
% <problem> <MOEADDE>
...
%--------------------------------------------------------------------------
    ...
    switch Operation
        ...
        case 'value'
            ...
            J1 = 3:2:D;
            J2 = 2:2:D;
            PopObj(:,1) = X(:,1)         + 2*mean((X(:,J1)-0.8*repmat(X(:,1),1,length(J1)).*cos(repmat(2*pi*X(:,1),1,length(J1))+repmat(J1*pi/D/3,N,1))).^2,2);
